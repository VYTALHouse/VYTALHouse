...
export async function POST(req: Request) {
  try {
    ...
    const auditEvent = {
      ...
      entity: "VYTAL House",
      type: "audit-event",
      name: `content-calendar POST interaction`,
      status: "logged",
      owner: "system",
      updatedAt: new Date().toISOString(),
      metadata: { action: "content-calendar" }
    };
...
export async function GET() {
  return NextResponse.json({ success: true, message: "content-calendar endpoint reachable." }, { status: 200 });
}
