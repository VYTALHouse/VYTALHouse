...
export async function POST(req: Request) {
  try {
    ...
    const auditEvent = {
      ...
      entity: "VYTAL House",
      type: "audit-event",
      name: `vendor-intake POST interaction`,
      status: "logged",
      owner: "system",
      updatedAt: new Date().toISOString(),
      metadata: { action: "vendor-intake" }
    };
...
export async function GET() {
  return NextResponse.json({ success: true, message: "vendor-intake endpoint reachable." }, { status: 200 });
}
